"""재구매 장바구니 챔피언(reorder-champion-v3) 화면. 저장된 집계만 싣고 외부 조회는 하지 않는다."""
import csv
import io
import json
...
def build(metrics=METRICS, output=OUTPUT):
    ...
    intro = (
        f"**{m['version']} · 등록 {m['as_of']} · 검증 {m['fold_range']} · {m['rows']:,}건**\n\n"
        ...
        "**담는 날은 기존 규칙(예측 재구매일 7일 전 이전 월요일)을 그대로 씁니다.** 바뀐 것은 네 가지입니다 — "
        "① 수량을 매장 단위 장바구니 모델 값으로 교체, ② 매장 주문 리듬 예측이 더 이르면 최대 7일까지 앞당김, "
        "③ 간격 예측을 분위 0.4로 재학습, ④ 그 고객×품목의 과거 예측 오차만큼 담는 날을 당기는 짝별 보정"
        f"(앞당김만 허용, 한도 7일, 검증 건의 {m['adjusted_share']:.0%}에 적용).\n\n"
    ...
    panels = [
        ...
        text_panel(8, "확인하지 않은 것", (
            ...
            "- 간격 재학습은 단일 XGBoost입니다. 라우터 구조(stable·speeding_up 전용 모델)로도 재학습해 비교했으나 "
            "21일 안 적중이 62.0% → 62.1%로 사실상 같았습니다. 학습 표본이 stable 1,076건, speeding_up 250건뿐이라 "
            "전용 모델이 붙는 것은 전체의 7%입니다. 기존 라우터의 TabPFN 분기는 재현하지 않았습니다.\n"
            "- 분위 0.4는 학습 기간 안쪽(2025-11~12)에서만 독립적으로 다시 골라 같은 값이 나왔습니다. 선택 편향은 해소되었습니다.\n"
            ...
            43, h=9)]
    dashboard = {"uid": "reorder-champion", "title": "재구매 장바구니 챔피언 · reorder-champion-v3",
                 "tags": ["forecasting", "champion"], "timezone": "Asia/Seoul", "schemaVersion": 39,
                 "version": 3, "editable": False, "refresh": "", "panels": panels,
